...
function tokenize(text: string): string[] {
  return text
    ...
    .replace(/[^\w\s\-\.]/g, ' ')
    .split(/\s+/)
    .filter((t) => t.length > 1 && !FR_STOP_WORDS.has(t))
}
